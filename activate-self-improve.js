 ...
 *
 * 配置说明：
 * - MEMORY_DIR: 记忆目录路径 (C:\Users\17589\.openclaw\workspace\memory)
 * - SELF_IMPROVE_PY: Python 自我改进脚本路径
 * - IMPROVEMENT_LOG: 改进日志文件路径 (self-improvement.md)
 * - METRICS_FILE: 性能指标 JSON 文件路径 (metrics/performance.json)
 *
 * 用法：
 ...
 *
 * 设计思路：
 * 为什么支持 task_success 和 task_error 两种模式？
 * - 成功时记录：什么方法有效，便于复用
 * - 失败时记录：什么问题需要改进，便于避免
 * - 两者结合 = 完整的经验积累系统
 *
 * 为什么性能指标单独存 JSON 而不是 Markdown？
 * - JSON 便于程序读取和图表生成
 * - 可以计算趋势、平均值等统计
 * - Markdown 用于人类阅读的总结报告
 * - 数据 (JSON)+ 报告 (MD) 分离，各司其职
 *
 * 为什么设计成命令行触发而不是自动触发？
 * - 命令行：灵活，可以手动触发
 * - 自动触发：可能在不合适时机执行
 ...
 *
 * 业务含义：
 * - task_success: 任务成功标记，记录成功经验和有效方法
 * - task_error: 任务错误标记，记录失败原因和改进方向
 * - IMPROVEMENT_LOG: 扎克系统改进日志，记录所有学习和优化过程
 * - METRICS_FILE: 性能指标文件，追踪任务执行效率变化
 * - 自我改进系统：扎克通过积累经验不断提升能力的核心机制
 *
 * 性能特征：
 * - 执行耗时：<100ms/次（文件写入操作）
 * - 内存占用：<5MB（轻量级脚本）
