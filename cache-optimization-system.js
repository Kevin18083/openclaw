 ...
 *
 * 设计思路：
 * 为什么分 3 类模板（basic/code/analysis）？
 * - basic: 基础问答，简单直接，不需要复杂格式
 * - code: 代码任务，需要结构化输出和最佳实践
 * - analysis: 分析任务，需要数据支撑和结论先行
 * - 分类目的：针对不同场景优化格式，提高缓存命中率
 *
 * 为什么模板要 20+ 个？
 * - 覆盖常见使用场景
 * - 场景越细，命中率越高
 ...
 *
 * 业务含义：
 * - TemplateLibrary: 模板库，存储所有场景的预定义模板
 * - basic: 基础问答模板（天气、事实、计算等）
 * - code: 代码任务模板（写函数、调试、重构等）
 * - analysis: 分析任务模板（数据分析、报告、对比等）
 * - 命中率：缓存复用的百分比，越高越省钱
 *
 * 性能特征：
 * - 模板查找耗时：<1ms（对象属性访问）
